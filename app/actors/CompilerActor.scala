...
class CompilerActor @Inject()(out: ActorRef, tokenId: String)(
  ...
  def checkForSuccess(programState: ProgramState, frame: Frame): Boolean =
  ...
  def createLastFrame(programState: ProgramState, frame: Frame) = {
    ...
    // Update stats
    // Add stats and step data to last frame
    for {
      ...
      for {
        ...
        (currentCompiler.flatMap(_.leftoverFrame), executeSomeFrames) match {
          case (None, List(frame)) =>
            ...
            } yield sendFrames(programState, lastFrame)
          ...
          case (None, leadingFrames :+ last) if programCompleted =>
            ...
            } yield sendFrames(programState, createFrames(leadingFrames) ++ lastFrame)
          ...
          case (Some(leftover), leadingFrames :+ last) if programCompleted =>
            ...
            for {
              lastFrame <- createLastFrame(programState, last)
            } yield sendFrames(programState, createFrames(leftover +: leadingFrames) ++ lastFrame)
          ...
          case (Some(leftover), List(last)) =>
            ...
            for {
              lastFrame <- createLastFrame(programState, last)
            } yield sendFrames(programState, createFrames(List(leftover)) ++ lastFrame)
          case (None, Nil) =>
