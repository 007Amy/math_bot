  ...
  override def receive: Receive = {
    ...
    case CompilerContinue(steps, problem) =>
      ...
      for {
        ...
        // The list could be empty, which means the program has finished running.
        if (executeSomeFrames.nonEmpty) {
          ...
          // Filter out any frames that are not robot related, like a function call.
          val clientFrames = executeSomeFrames
            .filter(f => f.robotLocation.isDefined)
            .map(f => ClientFrame(f))

          // If the program completed successfully. Then create a special client frame with the success frame.
          val lastFrame =
            if (success) {
