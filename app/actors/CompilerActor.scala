...
import model.models.{ GridMap, Problem }
...
class CompilerActor @Inject()(out: ActorRef, tokenId: String)(
  ...
  def sendFrames(programState: ProgramState, leadingFrames : List[Frame]) ={
    ...
    out ! CompilerOutput(deduped, programState.grid.problem)
  }

  def sendLastFrame(programState: ProgramState, frame : Frame)= {
    ...
    {
      out ! CompilerOutput(List(MathBotCompiler.ClientFrame.success(frame)), programState.grid.problem)
      statsActor ! UpdateStats(success = true, tokenId)
    } else {
      out ! CompilerOutput(List(MathBotCompiler.ClientFrame.failure(frame)), programState.grid.problem)
    }
  ...
  override def receive: Receive = {
    ...
    case CompilerContinue(steps, problem) =>
      ...
      for {
        ...
        // if a query never responds with a frame.
        (currentCompiler.flatMap(_.leftoverFrame), executeSomeFrames) match {
          case (None, List(frame)) =>
            // When the compiler generates a single frame, we assume its the last frame.
            sendLastFrame(programState, frame)
          case (None, leadingFrames :+ last) =>
            // More than one frame, we execute the leading frames and save the last frame for the next request
            sendFrames(programState, leadingFrames)
          ...
          case (Some(leftover), leadingFrames :+ last) =>
            ...
            sendFrames(programState, leftover +: leadingFrames)
            currentCompiler = currentCompiler.map(_.copy(leftoverFrame = Some(last)))
          case (Some(leftover), Nil) =>
            // The compiler generates no more frames, so the leftover is now the last
            sendLastFrame(programState, leftover)
          case (Some(leftover), List(last)) =>
            // The compiler generated one more frame, and there was one leftover, so run
            // the leftover and save the last frame for last call from the client.
            sendFrames(programState, List(leftover))
            currentCompiler = currentCompiler.map(_.copy(leftoverFrame = Some(last)))
          case (None, Nil) =>
